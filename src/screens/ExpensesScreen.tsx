...
import { Plus, TrendingDown, DollarSign, Trash2, X } from 'lucide-react-native'
...
export default function ExpensesScreen() {
  ...
  const expenseTransactions = useMemo(
    () => [...state.transactions.filter(t => t.type === 'expense')].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()),
    [state.transactions]
  )
  const totalExpenses = useMemo(() => expenseTransactions.reduce((s, t) => s + t.amount, 0), [expenseTransactions])
  ...
  const handleSubmit = async () => {
    ...
    await addTransaction({ type: 'expense', amount: parseFloat(amount), description, category, date })
      ...
      <ScrollView style={s.scroll} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        <View style={s.header}>
          <View>
            ...
            <Text style={s.pageSubtitle}>Monitor your spending</Text>
          </View>
          <TouchableOpacity style={[s.addBtn, { backgroundColor: colors.danger }]} onPress={() => setShowForm(true)}>
            <Plus size={20} color={colors.white} />
          </TouchableOpacity>
        </View>

        <View style={[s.totalCard, { borderLeftColor: colors.danger }]}>
          <View style={s.totalCardContent}>
            <View>
              <Text style={s.totalLabel}>Total Expenses</Text>
              <Text style={[s.totalValue, { color: colors.danger }]}>{formatCurrency(totalExpenses, user)}</Text>
              <View style={s.row}>
                <TrendingDown size={14} color={colors.danger} />
                <Text style={[s.totalSub, { marginLeft: 4 }]}>All time</Text>
              </View>
            </View>
            <View style={[s.totalIcon, { backgroundColor: colors.dangerLight }]}>
        ...
        <View style={s.card}>
          <Text style={s.cardTitle}>Expense History</Text>
          {expenseTransactions.length === 0 ? (
            <View style={s.empty}>
              <TrendingDown size={36} color={colors.gray300} />
              <Text style={s.emptyText}>No expenses yet</Text>
              <Text style={s.emptySubtext}>Add your first expense</Text>
            </View>
          ) : (
            expenseTransactions.map(txn => {
              ...
              return (
                <View key={txn.id} style={s.txnRow}>
                  <View style={[s.txnIcon, { backgroundColor: (cat?.color || colors.danger) + '22' }]}>
                  ...
                  <View style={{ alignItems: 'flex-end', marginRight: 8 }}>
                    <Text style={[s.txnAmount, { color: colors.danger }]}>-{formatCurrency(txn.amount, user)}</Text>
      ...
      <Modal visible={showForm} animationType="slide" presentationStyle="pageSheet">
        <SafeAreaView style={s.modalSafe}>
          <ScrollView contentContainerStyle={s.modalContent} keyboardShouldPersistTaps="handled">
            ...
            <TextInput style={s.input} placeholder="e.g., Groceries, Gas" placeholderTextColor={colors.gray400} value={description} onChangeText={setDescription} />
            ...
            <TouchableOpacity style={[s.submitBtn, { backgroundColor: colors.danger }]} onPress={handleSubmit}>
