...
import { Plus, TrendingUp, DollarSign, Trash2, X } from 'lucide-react-native'
...
export default function IncomeScreen() {
  ...
  const incomeTransactions = useMemo(
    () => [...state.transactions.filter(t => t.type === 'income')].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()),
    [state.transactions]
  )
  const totalIncome = useMemo(() => incomeTransactions.reduce((s, t) => s + t.amount, 0), [incomeTransactions])
  const categoryOptions = useMemo(() => incomeCategories.map(c => ({ label: `${c.icon} ${c.name}`, value: c.name })), [incomeCategories])
  ...
  const handleSubmit = async () => {
    ...
    await addTransaction({ type: 'income', amount: parseFloat(amount), description, category, date })
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          <View>
            ...
            <Text style={styles.pageSubtitle}>Track your income sources</Text>
          </View>
          <TouchableOpacity style={[styles.addBtn, { backgroundColor: colors.success }]} onPress={() => setShowForm(true)}>
        ...
        <View style={[styles.totalCard, { borderLeftColor: colors.success }]}>
          <View style={styles.totalCardContent}>
            <View>
              ...
              <Text style={[styles.totalValue, { color: colors.success }]}>{formatCurrency(totalIncome, user)}</Text>
              <View style={styles.row}>
                <TrendingUp size={14} color={colors.success} />
                <Text style={[styles.totalSub, { marginLeft: 4 }]}>All time</Text>
              </View>
            </View>
            <View style={[styles.totalIcon, { backgroundColor: colors.successLight }]}>
        ...
        <View style={styles.card}>
          ...
          {incomeTransactions.length === 0 ? (
            <View style={styles.empty}>
              <TrendingUp size={36} color={colors.gray300} />
            ...
            incomeTransactions.map(txn => {
              ...
              return (
                <View key={txn.id} style={styles.txnRow}>
                  <View style={[styles.txnIcon, { backgroundColor: (cat?.color || colors.success) + '22' }]}>
                  ...
                  <View style={{ alignItems: 'flex-end', marginRight: 8 }}>
                    <Text style={[styles.txnAmount, { color: colors.success }]}>+{formatCurrency(txn.amount, user)}</Text>
                    <Text style={styles.txnDate}>{formatDate(txn.date, user)}</Text>
                  </View>
                  <TouchableOpacity onPress={() => handleDelete(txn.id)} style={styles.deleteBtn}>
      ...
      <Modal visible={showForm} animationType="slide" presentationStyle="pageSheet">
        <SafeAreaView style={styles.modalSafe}>
          <ScrollView contentContainerStyle={styles.modalContent} keyboardShouldPersistTaps="handled">
            ...
            <TextInput style={styles.input} placeholder="0.00" placeholderTextColor={colors.gray400} value={amount} onChangeText={setAmount} keyboardType="decimal-pad" />

            <Text style={[styles.fieldLabel, { marginTop: 16 }]}>Description</Text>
            <TextInput style={styles.input} placeholder="e.g., Salary, Freelance work" placeholderTextColor={colors.gray400} value={description} onChangeText={setDescription} />
            ...
            <TextInput style={styles.input} value={date} onChangeText={setDate} placeholder="YYYY-MM-DD" placeholderTextColor={colors.gray400} />
...
const styles = StyleSheet.create({
  ...
  totalCard: {
    ...
    borderLeftWidth: 4, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 3,
  },
